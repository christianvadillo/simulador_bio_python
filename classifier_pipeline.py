...
def load_dataset(full_path, frac=1):
    data = pd.read_csv(full_path)
    data = data.sample(frac=frac, random_state=1)
...
da = pd.read_csv('OOP_TEST/test1/df_da_3600_10_Jul_2020_09_44_34.csv', index_col=0, parse_dates=True)
mec = pd.read_csv('OOP_TEST/test1/df_mec_3600_10_Jul_2020_09_44_34.csv', index_col=0, parse_dates=True)
...
X, y = load_dataset('OOP_TEST/test1/df_da_3600_10_Jul_2020_09_44_34.csv')
# X, y = load_dataset('OOP_TEST/test1/df_mec_3600_10_Jul_2020_09_44_34.csv')
...
pipeline_rf = Pipeline(steps=[('s', StandardScaler()),
                              ('RF', RandomForestClassifier(n_estimators=100,
                                                            max_depth=5000,
                                                            max_samples=0.4))])
plot_learning_curve(pipeline_rf, title, X, y,
...
plot_learning_curve(pipeline_lr, title, X, y,
                    axes=axes[:, 2], ylim=(0.7, 1.01),
                    cv=cv, n_jobs=-1)
...
plot_learning_curve(pipeline_svc, title, X, y,
                    axes=axes[:, 3], ylim=(0.7, 1.01),
                    cv=cv, n_jobs=-1)
plt.show()
